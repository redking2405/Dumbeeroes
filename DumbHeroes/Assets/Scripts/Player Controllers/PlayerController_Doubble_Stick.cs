...
public class PlayerController_Doubble_Stick : MonoBehaviour
{
    ...
    void AimArm(Vector2 direction)
    {
        ...
        if (V_player.GetButtonUp("Grab"))
        {
            if (O_Lhand.connectedBody != null)
            {
                O_Lhand.connectedBody.velocity = ((Vector2)O_Lhand.transform.localPosition - V_previousMidpoinrPosL) / Time.deltaTime*2;
                O_Lhand.connectedBody = null;
                O_Lhand.enabled = false;
            }
            if (O_Rhand.connectedBody != null)
            {
                O_Rhand.connectedBody.velocity = ((Vector2)O_Rhand.transform.localPosition - V_previousMidpoinrPosR) / Time.deltaTime*2;
